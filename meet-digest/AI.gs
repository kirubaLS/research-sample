/**
 * One model call per MEETING, not per person. The model must see the whole
 * conversation to know that "Ravi said Priya should own the API contract" is
 * Priya's item — that line can never be found in Priya's own utterances.
 *
 * Gemini and Claude both get the same prompt and the same schema, so switching
 * AI_PROVIDER changes nothing about the output shape.
 */
...
/** Returns { <displayName>: digest }. */
function generateDigests(script, names) {
  const prompt = buildPrompt(script, names);
  const raw = CONFIG.provider === 'claude' ? callClaude(prompt) : callGemini(prompt);

  const out = {};
  (raw.digests || []).forEach(d => { out[d.person] = d; });
  return out;
}

/** Shared retry wrapper: one retry on rate-limit or server error. */
function fetchWithRetry(url, options) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    if (code !== 429 && code < 500) return res;   // not retryable
