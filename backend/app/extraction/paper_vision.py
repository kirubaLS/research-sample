...
SYSTEM = (
    ...
    "A map-skill or other question sometimes carries a second printing right after it, "
    "introduced by a note such as 'The following questions are for the Visually Impaired "
    "Candidates only, in lieu of Q. No. 19' -- this restates the SAME sub-items in words "
    "instead of 'locate and label', worth the same marks, not a second, later part of the "
    "question and not additional sub-items. Extract only the original block's sub-items "
    "(the one the note is 'in lieu of'), exactly like the bilingual duplicate above -- "
    "never both printings, and never let seeing two blocks that reuse the same sub-item "
    "numbers (19.1, 19.2, ...) cause you to drop or truncate the original block's own "
    "items instead. "
    ...
    "\n\n"
    "A third shape looks like the OR case but is not: the word OR (or its Hindi or Tamil "
    "equivalent) sits between two whole, separately numbered sub-items -- (i) ... OR ... "
    "(ii) ..., not two lettered options (a)/(b) inside one sub-item. choice_alt only "
    "applies when the alternative sits INSIDE a single sub-part (same question_no and "
    "sub_part, the first blank and the alternative 'b'); it cannot represent two "
    "different sub_part values. When OR instead separates two or more complete "
    "numbered/lettered sub-items, treat it exactly like GROUP MARKS above: set "
    "attempt_required to 1 (attempt one of them) on every sub-item joined by that OR, "
    "and its max_marks to the per-item mark from the group's own printed 'N x M = Total' "
    "(here N is 1). The presence of the word OR does not by itself mean choice_alt -- "
    "what matters is whether the alternative is a lettered option inside one sub-part, "
    "or a whole separate sub-item in its own right. "
    "\n\n"
