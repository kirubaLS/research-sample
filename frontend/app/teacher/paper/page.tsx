...
export default function PaperPage() {
  ...
  const loadPapers = useCallback(async () => {
    ...
    if (!key) return;
    try {
      // Scoped server-side to the subjects this teacher key holds a subject assignment
      // for -- never the whole school's papers, unlike the principal's listPapers().
      const { assessments } = await api.teacherPapers(key);
  ...
  useEffect(() => {
    const key = getApiKey();
    if (!key) return;
    // The subject picker for a *new* paper is narrowed to this teacher's own subject
    // assignments, taken from /admin/teacher/sections -- a class assignment names no
    // subject and grants no paper-authoring right, so it contributes nothing here.
    Promise.all([api.subjects(key), api.teacherSections(key)])
      .then(([{ subjects: allSubjects }, { sections }]) => {
