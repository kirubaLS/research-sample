  ...
  return (
    <div>
      ...
      <div className="row" style={{ justifyContent: "space-between", padding: "10px 0" }}>
        <span
          className={`tag ${
            quality?.band === "green" ? "tag--green" : quality?.band === "amber" ? "tag--gold" : "tag--risk"
          }`}
        >
      ...
      <div className="scanner-strip">
        {pages.map((p) => (
          <div
            ...
            style={{
              borderColor:
                p.quality.band === "green" ? "var(--brand-green)" : p.quality.band === "amber" ? "var(--brand-gold)" : "var(--risk)",
            }}
          >
            <span className="scanner-thumb__n">{p.index + 1}</span>
            {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      <style jsx>{`
        .scanner-strip { display: flex; gap: 8px; flex-wrap: wrap; margin-top: 12px; }
        .scanner-thumb { position: relative; width: 64px; border: 2px solid var(--line); border-radius: 8px; padding: 3px; }
        .scanner-thumb img { width: 100%; border-radius: 5px; display: block; }
        .scanner-thumb__n {
          position: absolute; top: 2px; left: 2px; background: rgba(0,0,0,.65); color: #fff;
