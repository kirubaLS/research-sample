...
/** One student in a drill-down list, always a link through to their own report, so a
 * number on the overview is never a dead end. */
export function StudentRow({ student, showSection, meta }: { student: DrillStudent; showSection: boolean; meta?: string }) {
  return (
    <Link href={`/principal/classes/${student.section_id}/${student.student_id}`} className="subject-row">
      <div>
        <div className="strong">{student.name}</div>
        <div className="small muted">
          Roll {student.roll_no}
          {showSection && student.section_label ? ` · ${student.section_label}` : ""}
        </div>
      </div>
...
export interface DrillDown {
  ...
  metaFor?: (student: DrillStudent) => string;
}
            ...
            <div className="drawer__body">
              ...
              <div style={{ display: "grid", gap: 8 }}>
                {drill.students.map((s, i) => (
                  <div key={s.student_id} className="reveal" style={{ "--d": `${Math.min(i, 14) * 22}ms` } as React.CSSProperties}>
